                            ...
                            @foreach($rows as $row)
                                ...
                                @foreach($index_fields as $labels => $fields)
                                    ...
                                    @elseif(strpos($row->$fields, '.png') || strpos($row->$fields, '.jpg') || strpos($row->$fields, '.jpeg'))
                                        <td data-toggle="modal" data-target="#myModal{{$row->id}}"> <img class="img_preview" src="{{ asset('images/'.$route.'/'.$row->image ? 'images/'.$route.'/'.$row->image : 'images/'.$route.'/'.$row->logo) }}" style="height: 200px;width: 300px;"> </td>
                                        <div id="myModal{{$row->id}}" class="modal fade" role="img">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-body">
                                                        <img data-toggle="modal" data-target="#myModal{{$row->id}}" class="img-preview" src="{{ asset('images/'.$route.'/'.$row->image ? 'images/'.$route.'/'.$row->image : 'images/'.$route.'/'.$row->logo) }}" style="max-height: 500px">
                                                    </div>
                                ...
                                @endforeach
                                    @if($language==2)
                                        @foreach($descriptions as $label=>$description)
                                            @foreach($languages as $lang)
                                                <td> {{$modelDescription->where(['language_id'=>$lang->id,$route.'_id'=>$row->id])->value($description)}} </td>
                                            @endforeach
                                        @endforeach
                                    @endif
                                    <th> {!! $row->activate() !!} </th>
