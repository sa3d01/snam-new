                            ...
                            @foreach($rows as $row)
                                ...
                                @foreach($index_fields as $labels => $fields)
                                    ...
                                    @elseif(strpos($row->$fields, '.png') || strpos($row->$fields, '.jpg') || strpos($row->$fields, '.jpeg'))
                                            ...
                                            @else
                                                <td data-toggle="modal" data-target="#myModal{{$row->id}}"> <img class="img_preview" src="{{ $row->logo }}" style="height: 200px;width: 300px;"> </td>
                                                <div id="myModal{{$row->id}}" class="modal fade" role="img">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-body">
                                                                <img data-toggle="modal" data-target="#myModal{{$row->id}}" class="img-preview" src="{{$row->logo}}" style="max-height: 500px">
                                                            </div>
                                                            <div class="modal-footer">
                                                                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                                                            </div>
                                    ...
                                    @elseif(substr($s1, -strlen($s2))==$s2)
                                        @php($cat=\App\Models\Category::find($row->$fields))
                                        <td>{{$cat->name['ar']}}</td>
                                    @else
                                        <td> {{ $row->$fields }} </td>
                                    @endif
